...
export function installLocalModule(
  ...
  router
    .post("/api/local/bootstrap", async (request: IRequestStrict, env: ApiEnv) => {
      const body = await service.bootstrap(await request.json(), env);
      return jsonResponse(body, { status: 201 });
    })
    .post("/api/local/tokens/rotate", async (request: IRequestStrict, env: ApiEnv) => {
      await requireLocalToken(request, env, repository);
      return jsonResponse(await service.rotate(await request.json(), env));
    })
    .post("/api/local/tokens/revoke", async (request: IRequestStrict, env: ApiEnv) => {
      await requireLocalToken(request, env, repository);
